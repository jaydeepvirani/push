...
/**
 * RunTime Permission Manager for marshmallow devices
 */
public class RunTimePermissionManager {
   ...
   * display name
   */
  public void buildPermissionList(HashMap<String, String> permissionsMap, int code) {
    ...
    if (!isArrayListEmpty(manifestPermissionKey)) {
      ...
      if (!isArrayListEmpty(permissionsNeededName)) {
        ...
        if (hasCallbackAvailable()) {
          ...
          callbackRunTimePermission.showRationalDialog(buildMessage, code);
        }
      } else {
        // permission granted array list is empty that means all permission is granted to this application
        if (hasCallbackAvailable()) callbackRunTimePermission.requestAllPermissionGranted(code);
      }
    }
  }
    ...
    if (mRequestCode == requestCode) {
      ...
      if (hasCallbackAvailable()) {
        if (isAllPermissionGranted) {
          callbackRunTimePermission.requestAllPermissionGranted(mRequestCode);
        } else {
          if (isNeverAsk) {
            openSettingScreen();
          } else {
            ...
            callbackRunTimePermission.deniedPermission(buildMessage, mRequestCode);
          }
        }
      }
      mRequestCode = -1;
  ...
  public interface callbackRunTimePermission {
    void showRationalDialog(String message, int code);

    void deniedPermission(String deniedPermission, int code);

    void requestAllPermissionGranted(int requestCode);
  }
}